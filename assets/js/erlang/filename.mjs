...
"basename/1": (atomOrBinaryOrList) => {
	...
	if (Type.isBitstring(atomOrBinaryOrList) && !Type.isBinary(atomOrBinaryOrList)) {
	...
	if (Type.isTuple(atomOrBinaryOrList) || (!Type.isAtom(atomOrBinaryOrList) && !Type.isBinary(atomOrBinaryOrList))) {
		Interpreter.raiseFunctionClauseError(
			Interpreter.buildFunctionClauseErrorMsg(":filename.do_flatten/2", [
				atomOrBinaryOrList,
				Type.list([]),
	...
	let binary = isAtom ? Erlang["atom_to_binary/1"](atomOrBinaryOrList) : atomOrBinaryOrList;
