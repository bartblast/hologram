defmodule Hologram.DB.EntityOperations do
      ...
      case Entity.validate(entity) do
        :ok ->
          ...
          Connection.transaction(fn ->
            ...
            Oplog.append([put_effect(stamped_entity)])
    ...
    transaction_result =
      Connection.transaction(fn ->
        ...
        case delete_entity_row(entity_type, table, encoded_id) do
          nil -> :ok
          entity -> Oplog.append([delete_effect(entity)])
        end
  ...
  # What a row held, as the oplog records it: every column it carries except the sort-key
  ...
  # What the entity now is, as the oplog records it, with the revisions the write set.
