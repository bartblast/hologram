defmodule Hologram.Compiler.CallGraph do
  ...
  # Skip traversing __impl__/1 clause body. This function is generated by defimpl
  # and its body contains module atoms (the :for and :protocol values) that are metadata,
  # not real dependencies. Without this, reaching any protocol implementation would
  # pull in the target module's entire function tree via the AtomType handler.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :__impl__, arity: 1},
        %Context{from_vertex: from_vertex}
      ) do
    add_edge(call_graph, from_vertex, {from_vertex, :__impl__, 1})
  end

  # Suppress module vertex edges in __struct__/1 clause body. This function is generated by
  # defstruct and its body contains module atoms as default field values in the initial struct
  # map (e.g. calendar: Calendar.ISO in Date). These are data defaults, not real dependencies.
  # Without this, any struct with a module atom as a default value would pull in that module's
  # entire function tree. MFA edges (Enum.reduce/3, Map.merge/2, etc.) are not affected.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :__struct__, arity: 1, clause: clause},
        %Context{from_vertex: from_vertex} = context
      ) do
    fun_def_vertex = {from_vertex, :__struct__, 1}
  ...
  # Skip traversing impl_for/1 clause body. This function is generated by defprotocol
  # and its body contains module atoms for ALL implementation modules (return values for
  # type-based dispatch). Without this, every implementation module vertex would be reached,
  # pulling in their entire function trees. The actual protocol dispatch is handled by
  # add_protocol_call_graph_edges/2.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :impl_for, arity: 1},
        %Context{from_vertex: from_vertex}
      ) do
    add_edge(call_graph, from_vertex, {from_vertex, :impl_for, 1})
  end

  # Skip traversing impl_for!/1 clause body. This function is generated by defprotocol
  # and its body contains a self-referential module atom (__MODULE__ in the
  ...
  # reachable. The actual protocol dispatch is handled by add_protocol_call_graph_edges/2.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :impl_for!, arity: 1},
        %Context{from_vertex: from_vertex}
  ...
  # Skip traversing __protocol__/1 clause body. This function is generated by defprotocol
  # and its body contains module atoms (consolidated implementations, the protocol module)
  # that are metadata, not real dependencies. Without this, reaching any protocol function
  # would pull in every consolidated implementation module's entire function tree.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :__protocol__, arity: 1},
        %Context{from_vertex: from_vertex}
      ) do
    add_edge(call_graph, from_vertex, {from_vertex, :__protocol__, 1})
  end

  # Skip traversing struct_impl_for/1 clause body. This function is generated by defprotocol
  # and its body contains module atoms for ALL struct-based implementation modules (both as
  # pattern matches and return values). Without this, every struct implementation module vertex
  # would be reached, pulling in their entire function trees. The actual protocol dispatch is
  # handled by add_protocol_call_graph_edges/2.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :struct_impl_for, arity: 1},
        %Context{from_vertex: from_vertex}
      ) do
