defmodule Hologram.Compiler.CallGraph do
  ...
  # Explicit edge to struct_impl_for/1 is added because impl_for/1 has a struct dispatch
  # clause that calls struct_impl_for/1, and that edge would otherwise be lost with the
  # skipped body traversal.
  def build(
        call_graph,
    ...
    call_graph
    |> add_edge(from_vertex, {from_vertex, :impl_for, 1})
    |> add_edge({from_vertex, :impl_for, 1}, {from_vertex, :struct_impl_for, 1})
  end
  ...
  # Explicit edge to impl_for/1 is added because impl_for!/1 always calls impl_for/1,
  # and that edge would otherwise be lost with the skipped body traversal.
  def build(
        call_graph,
        %IR.FunctionDefinition{name: :impl_for!, arity: 1},
        %Context{from_vertex: from_vertex}
    ...
    call_graph
    |> add_edge(from_vertex, {from_vertex, :impl_for!, 1})
    |> add_edge({from_vertex, :impl_for!, 1}, {from_vertex, :impl_for, 1})
  end
