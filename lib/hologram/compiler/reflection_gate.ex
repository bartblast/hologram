defmodule Hologram.Compiler.ReflectionGate do
  ...
  #
  # A call on a module that is the calling function's parameter opens its reflection function only
  # when some caller can pass that parameter something other than a module written in the code.
  # The callers are followed up the reached functions: a caller passing an atom it names keeps the
  # call closed, one passing its own parameter is followed in turn, and one passing anything else
  # opens it. Anything the walk cannot see opens it too: a function the runtime's JavaScript calls
  # (an entry), one no reached function calls, a module vertex among the callers (the runtime calls
  # a component's functions), and a caller whose code shows no call of the function (a capture, a
  # protocol dispatch, a call the graph was given by hand).
  ...
  # (see CallGraph.runtime_reflection/2), and the IR PLT the callers' code is read from, where a
  # module it does not hold yet is put once read.
  @type t :: %{ir_plt: PLT.t(), runtime: CallGraph.runtime_reflection()}
  ...
  graph, with arguments the graph cannot see): those a reflection site among the vertices calls on a
  module that is not its function's parameter, those a site calls on a parameter some caller can
  pass anything else than a module written in the code, and those the runtime opens. With no gate,
  ...
  def open_functions(_graph, _reached_vertices, _entries, nil) do
    MapSet.new(ReflectionSites.functions())
  end
  ...
  def open_functions(graph, reached_vertices, entries, gate) do
    ...
    for {:reflection_site, function, name, arity, kind} <- reached_vertices,
  ...
  defp open_caller?({module, _function, _arity} = caller, function, index, context, seen) do
    case clauses(caller, context.ir_plt) do
      ...
      clauses ->
        calls = Enum.flat_map(clauses, &ReflectionSites.call_args(&1, module, function))
