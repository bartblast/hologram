...
- Reflection functions (`__struct__/0,1`, `__changeset__/0`, `__schema__/1,2`) called on a named module (`User.__changeset__()`) ship like any other function. Called on a module the code does not name (`mod.__changeset__()`, `apply(mod, :__schema__, [:fields])`), they ship with a page only for the types the page can see, the same types protocol implementations ship for (see above). A call through `apply/3` with a function name known only at runtime is not detected and fails on the client - name the function, or the module, in client-reachable code.
