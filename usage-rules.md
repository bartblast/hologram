...
- Protocol implementations ship to the browser only for types the compiler can see reaching the client: structs referenced in client code, and structs that `init`/command code or code that broadcasts an action hands to the client (in state, context, action or command params, or broadcast params). A struct server code only uses on the way (turned into a string, counted, validated) ships nothing. A struct that `init`/command code puts in the session, the cookies or the stash counts as handed to the client, since code that runs later can read it back (a later request for the session and cookies, later in the same request for the stash); one that middleware or a controller puts there counts only when the code reading it names its module (e.g. `%User{} = user`). Where the compiler loses track of a value (e.g. at a call into Erlang code or a message from another process), it assumes the value can hold anything passed into the code it lost track in and any struct created in that code (e.g. `Repo.get!(User, id)` ships `User`'s implementations, since `User` was passed in). A type resolved purely at runtime (e.g. from a string or external data) won't have its implementations bundled - reference its struct module anywhere in client-reachable code if needed.
- Reflection functions (`__struct__/0,1`, `__changeset__/0`, `__schema__/1,2`) called on a named module (`User.__changeset__()`) ship like any other function. Called on a module the code does not name (`mod.__changeset__()`, `apply(mod, :__schema__, [:fields])`), they ship with a page only for the types the page can see, the same types protocol implementations ship for (see above). A call through `apply/3` with a function name known only at runtime is not detected, so a reflection function reached only that way raises `UndefinedFunctionError` on the client - write the function name in the call (`apply(mod, :__schema__, args)`), or call the function on its module somewhere in client-reachable code. Naming the module alone is not enough.
...
- A dynamic component's module must appear as a literal in code the compiler can see - a template, a client-reachable function, or the `init/3`, `command/3` or broadcasting code that hands it to the client (in state, action params or broadcast params). A module that server code names but never hands to the client is not bundled. Modules conjured at runtime (`Module.concat/1`, `String.to_existing_atom/1`, values from the database or config) are not bundled and raise on the client.
...
- The same event can be bound more than once on one element - each `$event` attribute is an independent binding with its own modifiers, timer, and window, and bindings for the same event run in written order. Pairing a plain binding with a debounced one keeps state current on every event while debouncing only the expensive work: `$change="update_query" $change.debounce(300)="run_search"` (the typeahead pattern). Windows can also be layered - `debounce(250)` next to `debounce(1000)` stages cheap and expensive work.
...
- A pending debounced dispatch resolves early at lifecycle boundaries: it fires immediately when the bound element loses focus (the burst is over) and when the enclosing form is submitted (the submit action observes fresh state, with no trailing dispatch afterward), and it is dropped without firing when the user navigates to another page. Debounce is a pure timing concern - it changes when the dispatch happens, never what the app observes at a boundary.
...
- Like a pending debounced dispatch, a trailing dispatch held inside an open throttle window is dropped when the user navigates to another page - it never fires into a page it was not meant for.
- Append `once` to fire a binding a single time then stop - one-shot buttons, `$submit.once` against a double-submit, lazy-load on first reach: `$click.once="confirm"`. Works on every event and combines with `debounce`/`throttle` (the single debounced or throttled dispatch fires, then it stops). A spent binding still suppresses the native default and stops propagation per its modifiers - only the action stops dispatching. It is a precise guarantee scoped to the binding's target: an element binding re-arms only when the element is removed and re-added as a fresh DOM node (a re-render keeps it spent), while a `<window>`/`<document>` binding keys on the global target and so fires once for the page without re-arming. For at-most-once over the page lifetime regardless of re-creation, drive it from state instead.
