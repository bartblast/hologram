...
- Append `once` to fire a binding a single time then stop - one-shot buttons, `$submit.once` against a double-submit, lazy-load on first reach: `$click.once="confirm"`. Works on every event and combines with `debounce`/`throttle` (the single debounced or throttled dispatch fires, then it stops). A spent binding still suppresses the native default and stops propagation per its modifiers - only the action stops dispatching. It is a precise guarantee scoped to the element instance, not the page: a re-render keeps a fired binding spent, but removing and re-adding the element makes a fresh DOM node that re-arms it. For at-most-once over the whole page lifetime regardless of re-creation, drive it from state instead.
