...
- `try` (with `rescue`/`catch`/`else`/`after`), `raise`, `throw`, and `exit` work client-side, but `reraise` and `__STACKTRACE__` are not supported yet (the client has no stacktraces).
