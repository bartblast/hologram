...
- Reflection functions (`__struct__/0,1`, `__changeset__/0`, `__schema__/1,2`) ship with a page only when its client code or the runtime can call them on a module it does not name (`mod.__changeset__()`, `apply(mod, :__schema__, [:fields])`). A call through `apply/3` with a function name known only at runtime is not detected and fails on the client - name the function, or the module, in client-reachable code.
