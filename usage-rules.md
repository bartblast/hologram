...
- Each reach binding watches the container's existing edge child (first child for top/left, last for bottom/right), so a list fires as its first/last item nears and nothing is injected. A very tall edge item fires as soon as it starts to appear (before the true edge), and an empty container never fires - if you need the exact edge or an empty list to fire, add a sentinel element such as `<div class="sentinel"></div>` at that edge as the watched child.
