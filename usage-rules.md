...
- Bind events with `$` prefix: `$click`, `$click_outside`, `$change`, `$submit`, `$blur`, `$focus`, `$key_down`, `$key_up`, `$mouse_move`, `$pointer_down`, `$pointer_up`, `$pointer_move`, `$pointer_cancel`, `$drag_start`, `$drag`, `$drag_enter`, `$drag_over`, `$drag_leave`, `$drag_end`, `$drop`, `$resize`, `$scroll`, `$select`, `$transition_end`, `$transition_start`, `$transition_run`, `$transition_cancel`. **Not** `phx-click` or `phx-change`.
...
- Drag and drop events: `$drag_start`, `$drag`, `$drag_enter`, `$drag_over`, `$drag_leave`, `$drag_end`, `$drop`. Add `draggable="true"` to drag sources. For a drop target, bind `$drag_over` and `$drop` on the same element; browsers only deliver drops to targets that cancel drag-over behavior, and Hologram prevents default for `$drag_enter`, `$drag_over`, and `$drop` unless `allow_default` is set. `$drag_start`, `$drag`, `$drag_leave`, and `$drag_end` allow the browser default so native dragging is not canceled. `params.event` has the mouse coordinates, `alt_key`, `ctrl_key`, `meta_key`, `shift_key`, `button`, `buttons`, and `data_transfer` metadata: `%{drop_effect, effect_allowed, types, items}`. `data_transfer.items` contains `%{kind, type}` metadata only. Live `DataTransfer`, `DataTransferItem`, and `File` objects are not serialized to Elixir. `$drag` and `$drag_over` fire rapidly, so pair them with `.throttle(ms)` or `.debounce(ms)`.
...
- Append `allow_default` so Hologram does not call `preventDefault` for that binding, letting the browser's native default run while the action still dispatches: `$submit.allow_default="track"`. Hologram calls `preventDefault` by default only on `$submit` and drop-target drag events (`$drag_enter`, `$drag_over`, `$drop`), so `allow_default` is meaningful only there - e.g. a form that posts to an external endpoint you also want to track, or a drag target that should preserve the browser's native behavior. Composes with key filters, `stop_propagation`, `debounce`, and `throttle`.
- Append `prevent_default` so Hologram calls `preventDefault` for a binding - this is how you prevent the native default on events that allow it by default: `$key_down.enter.prevent_default="send_message"` fires the action without Enter inserting a newline or submitting the form. Only affects cancelable events (no-op on `$change`, `$input`, `$select`, `$scroll`, `$resize`, `$focus`, `$blur`, `$pointer_cancel`, `$transition_*`). It is the mirror of `allow_default`, and the two cannot combine on one binding (fails the build). Composes with key filters (only the matched key is prevented), `stop_propagation`, `debounce`, and `throttle`.
