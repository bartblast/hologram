defmodule HologramClusterTests.MixProject do
  ...
  defp deps do
    [
      ...
      {:hologram,
       ...
       ref: "d9e4be5999cb9b2abef832332b54f99492072ddc"},
      {:jason, "~> 1.0"},
      {:mint, "~> 1.0", only: :test},
      {:phoenix, "~> 1.7"},
      {:plug_cowboy, "~> 2.0"},
  ...
  def project do
    [
      ...
      # for these two, so even 2.20.0 with its cow_http_struct_hd hardening is still
      # flagged. Response splitting is mitigated by cowboy's CR/LF header validation.
