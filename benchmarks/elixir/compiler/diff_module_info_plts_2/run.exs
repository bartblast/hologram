alias Hologram.Benchmarks
alias Hologram.Compiler

Benchee.run(
  %{
    "no module changes" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(0, 0, 0)
       end},
    "1 module added" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(1, 0, 0)
       end},
    "1 module removed" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(0, 1, 0)
       end},
    "1 module edited" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(0, 0, 1)
       end},
    "100% modules added" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(1.0, 0.0, 0.0)
       end},
    "100% modules removed" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(0.0, 1.0, 0.0)
       end},
    "100% modules edited" =>
      {fn {old_module_info_plt, new_module_info_plt} ->
         Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
       end,
       before_scenario: fn _input ->
         Benchmarks.generate_module_info_plts(0.0, 0.0, 1.0)
       end},
    "33% added, 33% removed, 34% edited" => {
      fn {old_module_info_plt, new_module_info_plt} ->
        Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
      end,
      before_scenario: fn _input ->
        Benchmarks.generate_module_info_plts(0.33, 0.33, 0.34)
      end
    },
    "1% added, 1% removed, 1% edited" => {
      fn {old_module_info_plt, new_module_info_plt} ->
        Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
      end,
      before_scenario: fn _input ->
        Benchmarks.generate_module_info_plts(0.01, 0.01, 0.01)
      end
    },
    "10 added, 10 removed, 10 edited" => {
      fn {old_module_info_plt, new_module_info_plt} ->
        Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
      end,
      before_scenario: fn _input ->
        Benchmarks.generate_module_info_plts(10, 10, 10)
      end
    },
    "3 added, 3 removed, 3 edited" => {
      fn {old_module_info_plt, new_module_info_plt} ->
        Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
      end,
      before_scenario: fn _input ->
        Benchmarks.generate_module_info_plts(3, 3, 3)
      end
    },
    "1 added, 1 removed, 1 edited" => {
      fn {old_module_info_plt, new_module_info_plt} ->
        Compiler.diff_module_info_plts(old_module_info_plt, new_module_info_plt)
      end,
      before_scenario: fn _input ->
        Benchmarks.generate_module_info_plts(1, 1, 1)
      end
    }
  },
  formatters: [
    Benchee.Formatters.Console,
    {Benchee.Formatters.Markdown,
     description: "Hologram.Compiler.diff_module_info_plts/2",
     file: Path.join(__DIR__, "README.md")}
  ],
  time: 10
)
